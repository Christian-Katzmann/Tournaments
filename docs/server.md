...
Vanilla `node:http` server. No framework. Mirrors the Campaigns server's shape
(port discovery, static serving, registry, atomic writes) but for tournament
JSON files.
...
Default port: **4278** (Campaigns is 4178, +100 by convention). Falls back up to
`4278 + 50` if the port is in use.

The active port is written to `~/Library/Logs/Tournaments/server.port`, the PID
to `server.pid`, and a startup line to `server.log`. The skill discovers the
server by reading `server.port`.

The registry lives at
`~/Library/Application Support/Tournaments/registry.json` and is initialized as
`[]` on first run.
...
curl -X POST http://localhost:4278/api/registry \
  ...
  -d '{ "filePath": "/Users/me/Dev/Projects/Tournaments/tournaments/example.json" }'
# → { "id": "tour-…", "filePath": "/…/example.json" }
```
...
curl -X POST http://localhost:4278/api/tournament \
  -H 'content-type: application/json' \
  -d @./tournaments/example-typography.json
# → 201 { "id": "tour-…", "filePath": "/…/example-typography.json", "slug": "example-typography" }
```
