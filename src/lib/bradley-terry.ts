...
export interface BTComparison {
  /** The item id credited with a win. For ties we emit two half-weight rows. */
  winner: string;
  loser: string;
...
export interface BTOutput {
  ...
  /** SE on the rating scale. Null if the item has no games. */
  seRatings: Record<string, number | null>;
  ciLow: Record<string, number | null>;
  ciHigh: Record<string, number | null>;
  /** Comparisons involving the item (weighted, so ties count toward both). */
 ...
 *
 * Items that never appear in any comparison are still returned, with
...
export function computeBradleyTerry(
  ...
  // Initialize strengths to 1. Items with zero games stay at 1 forever.
...
export function votesToComparisons(
  ...
  for (const v of votes) {
    if (v.winnerItemId === v.loserItemId) continue;
    if (v.outcome === 'decisive') {
      out.push({ winner: v.winnerItemId, loser: v.loserItemId, weight: 1 });
    } else {
      out.push({ winner: v.winnerItemId, loser: v.loserItemId, weight: 0.5 });
      out.push({ winner: v.loserItemId, loser: v.winnerItemId, weight: 0.5 });
    }
  }
  return out;
}
